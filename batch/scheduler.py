...
class BatchScheduler:
    ...
    def _add_scheduled_jobs(self):
        ...
        self.scheduler.add_job(
            ...
            max_instances=1,
            coalesce=True,
            misfire_grace_time=self.config.scheduling.misfire_grace_time_status
        )
        ...
        self.scheduler.add_job(
            ...
            max_instances=1,
            coalesce=True,
            misfire_grace_time=self.config.scheduling.misfire_grace_time_history
        )
        
        # 日次クリーンアップジョブ - 設定された時刻に実行
        self.scheduler.add_job(
            func=self._cleanup_old_logs,
            trigger=CronTrigger(
                hour=self.config.scheduling.cleanup_time_hour, 
                minute=self.config.scheduling.cleanup_time_minute
            ),
            id='cleanup_logs',
            name='ログクリーンアップ',
            max_instances=1
        ...
        # ヘルスチェックジョブ - 設定された間隔で実行
        self.scheduler.add_job(
            func=self._health_check,
            trigger=IntervalTrigger(minutes=self.config.scheduling.health_check_interval),
            id='health_check',
            name='システムヘルスチェック',
            max_instances=1
        )
    ...
    def _cleanup_old_logs(self):
        ...
        try:
            ...
            cutoff_time = datetime.now() - timedelta(days=self.config.scheduling.max_log_retention_days)
